...
export const LESSON_CONTENT: Record<number, Partial<Record<TabOption, string>>> = {
  1: {
    [TabOption.COURSE]: `
      <h3 class="text-xl font-bold text-brand-red mb-4">Acessibilidade da Análise Corporal</h3>
      <p class="mb-4">Nesta aula inaugural, exploramos o mito de que a análise corporal é um dom reservado a poucos. A verdade é que a análise é uma <strong>ciência treinável</strong>, baseada na biologia e na neurociência.</p>
      <p class="mb-4">Você descobrirá como o formato do corpo explica o funcionamento da mente e por que qualquer pessoa dedicada pode dominar essa ferramenta para transformar vidas.</p>
      <ul class="list-disc list-inside text-neutral-400 space-y-2 mb-6">
        <li>A diferença entre intuição e técnica.</li>
        <li>Os 5 traços de caráter visíveis a olho nu.</li>
        <li>Como começar sem formação prévia em psicologia.</li>
      </ul>
      <p>Prepare-se para mudar sua visão sobre o comportamento humano.</p>
    `,
